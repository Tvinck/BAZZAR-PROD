...
function AppCard({ app, index }: { app: BazzarApp; index: number }) {
  ...
  const handlePurchase = async () => {
    ...
    setBuying(true)
    try {
      const res = await fetch('https://connect-4va6.vercel.app/api/shop/app-purchase', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ appId: app.id, udid, email: email.trim() || undefined }),
      })
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 'auto' }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 'var(--r-sm)',
              fontWeight: 600, fontSize: '0.85rem',
              fontFamily: 'inherit', cursor: buying ? 'wait' : 'pointer',
              opacity: buying ? 0.6 : 1,
              transition: 'opacity 200ms ease',
            }}
          >
            {buying ? 'Обработка...' : bought ? '✓ Добавлено' : isPaid ? `Купить · ${price} ₽` : 'Установить'}
          </button>
        </div>
      </motion.div>

      {/* Email modal for paid apps */}
      {showEmailModal && (
        <>
          <div
            onClick={() => setShowEmailModal(false)}
            style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(6px)', zIndex: 2000 }}
          />
          <div style={{
            position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
            width: 'min(380px, 90vw)', background: 'var(--bg-3, #1a1a2e)',
            border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20,
            padding: '28px 24px', zIndex: 2001, textAlign: 'center',
          }}>
            ...
            <input
              ...
              style={{
                ...
                color: '#fff', fontSize: '0.92rem', outline: 'none', fontFamily: 'inherit', marginBottom: 16,
              }}
            />
            <button
              onClick={() => { setShowEmailModal(false); handlePurchase() }}
              disabled={!email.trim()}
              style={{
