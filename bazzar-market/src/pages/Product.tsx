        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 28, alignItems: 'start' }} className="prod-grid">
          ...
          <div>
            <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}
              className="card" style={{ position: 'relative', height: 320, background: product.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 70% 20%, rgba(255,255,255,0.25), transparent 55%)' }} />
              <motion.span animate={{ y: [0, -14, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }} style={{ fontSize: '7rem', filter: 'drop-shadow(0 12px 30px rgba(0,0,0,0.5))' }}>{product.emoji || '🛍️'}</motion.span>
              {product.badge && <span className="badge badge-hot" style={{ position: 'absolute', top: 16, left: 16 }}>🔥 Хит продаж</span>}
            </motion.div>
            ...
            <div className="card" style={{ padding: 24, marginTop: 18 }}>
              ...
              {[['Игорь', 'Пришло моментально, всё работает. Беру не первый раз 👍'], ['Sofia', 'Дешевле чем в других местах, поддержка помогла с активацией.']].map(([n, t]) => (
                <div key={n} style={{ display: 'flex', gap: 12, padding: '14px 0', borderTop: '1px solid var(--hair)' }}>
                  <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--grad)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#fff', flexShrink: 0 }}>{n[0]}</div>
                  <div>
          ...
          <div style={{ position: 'sticky', top: 90 }}>
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 18 }}>
                {denominations.map((d, i) => (
                  <button key={d.label} onClick={() => setDenom(i)}
                    style={{ padding: '11px 12px', borderRadius: 12, textAlign: 'left', cursor: 'pointer', border: `1px solid ${denom === i ? 'rgba(124,92,255,0.6)' : 'var(--hair)'}`, background: denom === i ? 'rgba(124,92,255,0.12)' : 'var(--surface-2)', color: 'var(--text)' }}>
                    <div style={{ fontWeight: 700, fontSize: '0.86rem' }}>{d.label}</div>
                    <div style={{ fontSize: '0.8rem', color: denom === i ? '#c4b5ff' : 'var(--text-3)' }}>{Math.round(product.price * d.mult).toLocaleString('ru-RU')} ₽</div>
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '16px 0', borderTop: '1px solid var(--hair)', borderBottom: '1px solid var(--hair)', marginBottom: 18 }}>
                ...
                <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.8rem' }} className="grad-text">{total.toLocaleString('ru-RU')} ₽</span>
              </div>
