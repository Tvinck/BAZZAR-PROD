          ...
          <div>
            <div className="card" style={{ display: 'flex', gap: 12, padding: 12, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8, overflowX: 'auto' }}>
                {SORTS.map((s, i) => (
                  <button key={s} onClick={() => setSort(i)} className="chip" style={{ whiteSpace: 'nowrap', cursor: 'pointer', ...(sort === i ? { background: 'rgba(124,92,255,0.16)', border: '1px solid rgba(124,92,255,0.4)', color: '#c4b5ff' } : {}) }}>{s}</button>
                ))}
              </div>
            </div>
...
function catBtn(activeState: boolean): React.CSSProperties {
  return {
    ...
    padding: '11px 13px', borderRadius: 11, marginBottom: 4, border: 'none', textAlign: 'left',
    background: activeState ? 'rgba(124,92,255,0.14)' : 'transparent',
    color: activeState ? '#c4b5ff' : 'var(--text-2)', fontWeight: 600, fontSize: '0.88rem', cursor: 'pointer'
  }
}
