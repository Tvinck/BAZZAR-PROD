      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, alignItems: 'start' }} className="cart-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {items.map(i => (
              <div key={i.id} className="card" style={{ padding: 14, display: 'flex', gap: 14, alignItems: 'center' }}>
                <div style={{ width: 76, height: 76, borderRadius: 13, background: i.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', flexShrink: 0 }}>{i.emoji || '🛍️'}</div>
          ...
          <div className="card" style={{ padding: 22, position: 'sticky', top: 90 }}>
            ...
            {[['Товары', `${subtotal.toLocaleString('ru-RU')} ₽`], ['Скидка', '—'], ['Кэшбэк баллами', `+${cashback} ₽`]].map(([k, v], idx) => (
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: '0.9rem', color: idx === 2 ? 'var(--green)' : 'var(--text-2)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '16px 0', marginTop: 6, borderTop: '1px solid var(--hair)' }}>
              ...
              <span className="grad-text" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.7rem' }}>{total.toLocaleString('ru-RU')} ₽</span>
            </div>
            <button className="btn btn-primary" style={{ width: '100%', height: 52 }} onClick={() => navigate('/cabinet')}>Оформить заказ</button>
